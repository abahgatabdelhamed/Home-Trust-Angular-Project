...
export class RolesManagementComponent implements OnInit, AfterViewInit {
    ...
    constructor(private alertService: AlertService, private checkPermission:CheckPermissionsService,private translationService: AppTranslationService, private accountService: AccountService) {
    }
        ...
        this.accountService.getRolesAndPermissions()
         ...
         //       this.alertService.stopLoadingMessage();
                ...
                permissions.forEach((p:any) =>{
                    tempPermission=tempPermission.concat(tempPermission,p.featureCode)
                })
                ...
                this.allPermissions = this.checkPermission.getGroups(permissions,true);
                console.log('all',this.allPermissions)
            },
            error => {
               // this.alertService.stopLoadingMessage();
                this.loadingIndicator = false;

               /* this.alertService.showStickyMessage(this.gT("messages.loadError"), `Unable to retrieve roles from the server.\r\nErrors: "${Utilities.getHttpResponseMessage(error)}"`,
        ...
        this.accountService.deleteRole(row)
            ...
            error => {
              //  this.alertService.stopLoadingMessage();
                this.loadingIndicator = false;
/*
                this.alertService.showStickyMessage(this.gT("messages.deleteError"), `An error occured whilst deleting the role.\r\nError: "${Utilities.getHttpResponseMessage(error)}"`,
                    MessageSeverity.error, error);
            */          });
    }
