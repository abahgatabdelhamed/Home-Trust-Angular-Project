...
export class EndpointFactory {
    ...
    getLoginEndpoint<T>(userName: string, password: string): Observable<T> {
        ...
        console.log("endpoint ser  login url ",this.loginUrl);
    ...
    IsRegistered() {
        let endpointUrl = this.configurations.baseUrl + "/api/setting/IsRegistered";

        return this.http.get(endpointUrl)
            .catch(error => {
                return this.handleError(error, () => this.IsRegistered());
            });
    }
    ...
    protected handleError1(error, continuation: () => Observable<any>) {
